        ...
        while going:
            try:
                ...
                if m.content == resp['content']:
                    ...
                    time = humanize.precisedelta((end - start).total_seconds())
                    await ctx.send(f"{m.author.mention} typed it correct in `{time}` with **{int(time.total_seconds())/len(resp['content'])/5}** wpm.")
                    going = False
